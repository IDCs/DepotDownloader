...
namespace DepotDownloader
{
  ...
  public class ContentDownloaderAccountException : Exception
  {
    public ContentDownloaderAccountException(uint appId, string contentName)
      : base(String.Format("App {0} ({1}) is not available from this account.", appId, contentName)) { }
  }
  ...
  static class ContentDownloader
  {
    ...
    public static async Task DownloadPubfileAsync(uint appId, ulong publishedFileId)
    {
      ...
      {
        await DownloadAppAsync(Exec.Instance, appId, new List<(uint, ulong)> { (appId, details.hcontent_file) }, DEFAULT_BRANCH, null, null, null, false, true);
      }
      else
      {
    ...
    public static async Task DownloadUGCAsync(uint appId, ulong ugcId)
    {
      ...
      {
        await DownloadAppAsync(Exec.Instance, appId, new List<(uint, ulong)> { (appId, ugcId) }, DEFAULT_BRANCH, null, null, null, false, true);
      }
    }
    ...
    public static async Task DownloadAppAsync(CoreDelegates core, uint appId,
      List<(uint depotId, ulong manifestId)> depotManifestIds, string branch,
      string os, string arch, string language, bool lv, bool isUgc)
    {
...
;
    ...
    public static List<FileData> VerifyFileIntegrity(string[] filePaths, List<FileData> filesData)
    {
